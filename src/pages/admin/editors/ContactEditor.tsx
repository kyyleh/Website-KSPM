...
export function ContactEditor() {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        setData(res.data || contactFormConfig);
      } catch {
  ...
  const addContactInfo = () => {
    setData({ ...data, contactInfo: [...data.contactInfo, { icon: 'Info', label: '', value: '', subtext: '' }] });
  };
    ...
    <div className="space-y-6">
      ...
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          ...
          <button onClick={addContactInfo} className="flex items-center gap-1 text-amber-400 hover:text-amber-300 text-sm"><Plus className="w-4 h-4" /> Tambah</button>
        </div>
