...
export function EditorTentang({ setIsDirty }: { setIsDirty?: (dirty: boolean) => void }) {
  ...
  const SectionHeader = ({ title, sectionKey }: { title: string; sectionKey: string }) => (
    ...
    >
      <h3 className="font-script text-[11px] sm:text-xs text-[#a67e2a] font-bold tracking-[0.12em]">{title}</h3>
      {openSections[sectionKey] ? <ChevronUp size={16} className="text-neutral-500" /> : <ChevronDown size={16} className="text-neutral-500" />}
    </button>
  );
      ...
      <div className="space-y-3.5">
          ...
          <div className="bg-white border border-[#eae6dd] rounded-xl px-4 py-1.5 shadow-sm">
            ...
            {openSections.timeline && (
              <div className="pb-4 space-y-3 border-t border-[#eae6dd]/40 pt-3">
